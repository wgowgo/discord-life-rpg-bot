...
class WorkSystem {
    ...
    async initializeJobs() {
        ...
        for (const job of jobs) {
            // salary_max가 없으면 salary_min의 1.2배로 설정
            const salaryMax = job.salary_max || Math.round(job.salary_min * 1.2);
            
            await this.db.run(`
                INSERT OR IGNORE INTO jobs (name, category, salary_min, salary_max, required_education, required_stats, description)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            `, [job.name, job.category, job.salary_min, salaryMax, job.required_education, job.required_stats, job.description]);
        }
    ...
    async getJobList() {
        return await this.db.all('SELECT * FROM jobs ORDER BY salary_min ASC');
    }
